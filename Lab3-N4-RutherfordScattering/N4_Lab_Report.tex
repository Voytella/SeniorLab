...
seal when the valve is fully closed, this is due to a bad seal also
the closing valve was extremely tight and failed to close easily if at
all. Alpha particles have a very short lifetime in air compared to a
vacuum, when air enters the chamber the particles reach the ender of
the lifetime quicker. This poses a problem as a vacuum chamber that
leaks would systematically lower the rate of counts. This issue was
combated by maintaining the vacuum inside the chamber which was
accomplished by keeping the vacuum pump on for the duration of each
data set. Equipment noise will systematically increase the rate
through what is known as a false count. False counts occur when a
signal comprised of noise is counted as an alpha particle, due to the
fact that the equipment cannot tell the difference between the
signals. This issue is overcome by raising the threshold on the
dsicriminator to above the noise baseline which will help to avoid
including any noise in the rate. On the fourth day of data taking the
baseline for the background noise increased from the previous
days. This occurred without the setup being altered. This was most
likely due to a bad cable connection and the threshold was raised to
account for the increased baseline. It is important to note that the
signal to noise ratio was still sufficiently high enough where alpha
particle counts were not being discriminated out of the rate. There
was a risk that this threshold would discriminate against alpha
particles counts, if that occurred the rate would have been
systematically lowered as less of the true counts would appear in the
rate. The rate of the scatterers is dependent on the density of the
scatterers, which in turn is dependent on the thickness of the
foil. This parameter is assumed to be constant and if an error occurs
in the thickness of the foil then it would cause an error in the
density of scatterers and thus an error in the rate would occur as
well. A nonuniform foil would show the rate to be altered as a result
