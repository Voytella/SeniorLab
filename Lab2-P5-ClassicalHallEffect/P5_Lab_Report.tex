...
Hall effect. The Hall voltage can be measured with a Hall probe that is
...
V_{Hall} = \frac{IB}{net}
\end{align*}
\qq  The Hall Voltage is a result of the Lorentz force, as discussed above. In this derivation, $I$ is the current through the sample, $B$ is the
 applied magnetic field, $n$ is the charge carrier concentration, $e$
 is the elementary charge, and $t$ is the sample thickness.

\begin{align*}
F_B &= qvB & \text {This gives the Lorentz force} \\
F_E &= Eq = \Big(\frac{V_H}{d}\Big)qb\\
F_B &= F_E & \text{We equate the two forces, for equilibrium case} \\
qvB &= \Big(\frac{V_H}{d}\Big)q \\
vB &= \frac{V_H}{d} \\
V_H &= Bvd \\
v &= \frac{I}{neA} & \text{Substitute an expression for drift velocity}\\
V_H &= \frac{BId}{neA} \\
A &= td & \text{Substitute an expression for the sample area} \\
V_H &= \frac{BId}{netd} \\
V_H &= \frac{IB}{net} & \text{We arrive at the Hall voltage equation}\\
\end{align*}
...
systematic rather than random error. Sources of systematic error include energy lost due to internal resistance of the sample holder or an offset in the power supply.
...
and the applied magnetic field strength was somewhat quadratic, as expected. However, the data was not able to span a large range of magnetic field values due to safety concerns associated with a high current through the coils
...
The theoretical carrier concentration, \( p_{\text{theo}} \), is determined in a
...
field surrounding the copper coils over which the experiment took place. The proximity of the Hall effect probe was also limited because it had to be fixed outside the sample holder, rather than within its designated cavity.
...
within 2 $\sigma$ of the theoretical value $\alpha_{theo} = 0.6 $ V/A, thus well within the acceptable range of error. Note the apparent presence of a step function was due to the limited range of the voltmeter being of a similar order of magnitude as the measurements. Thus is was difficult to obtain a smooth and continuous curve.
...
The associated error in $E_{g_{exp}}$ can be computed from the error in the slope.
\begin{align*}
\delta_{Eg_{exp}} &= {138}\times {2k} \\
                  &= {138}\times {2 \times (8.63 \times 10^(-5))} \\
                  &= 0.024 eV \\
\end{align*}
...
The field strength was limited to 290 mT. Recall the following relationship for this task.
$$R_H = \frac{U_H}{B} \times \frac{d}{I}$$
...
\begin{align*}
  R_{\text{H}} &= b \frac{d}{I} \\
  R_{\text{H}} &= (0.26) \frac{(\num{1e-3})}{(\num{30e-3})} \\
  R_{\text{H}} &= \num{0.0087}\si{\cubic\meter\per\ampere\per\second} \\
\end{align*}
...
\begin{align*}
  \delta R_{\text{H}} &= \delta b \frac{d}{I} \\
  \delta R_{\text{H}} &= (\num{3.5e-3}) (\num{0.0333}) \\
  \delta R_{\text{H}} &= 1.2 \times 10^{-4} \text{m}^3 \text{A}^{-1} \text{s}^{-1} \\
\end{align*}

\newpage
% Discrepancy compared to theory
\qq The theoretical value of the Hall constant is $4.8 \times 10^{-3}$ m$^3$/As. Therefore, the discrepancy
between our experimental and this theoretical value follows as shown below.

\begin{align*}
  \Delta_{R_{\text{H}}} &= | R_{\text{H}_{\text{theo}}} - R_{\text{H}_{\text{exp}}} | \\
  \Delta_{R_{\text{H}}} &= | (\num{4.8e-3}) - (\num{8.7e-3}) | \\
  \Delta_{R_{\text{H}}} &= 3.9 \times 10^{-3} m^3 /As \\
\end{align*}
...
n_{exp} &= \frac{1}{e \times R_{H_{exp}}} \\
	&= \frac{1}{(1.6 \times 10^{-19} As) \times (8.7 \times 10^{-3} A^{-1}s^{-1})} \\
	&= 7.2 * 10^{20}
\end{align*}

% Experimental error
The associated error in $n_{exp}$ is calculated as follows. This will also yield the value of $\sigma$, since the theoretical value used has no associated uncertainty.
\begin{align*}
\delta_n &= n_{exp} \times \frac{\delta R_{H{exp}}}{R_{H{exp}}} \\
&= (7.2 \times 10^{20}) \times \frac{1.2 \times 10^{-4}}{8.7 \times 10^{-3}} \\
&= 9.9 \times 10^{18} 
\end{align*}
...
n_{theo} &= \frac{1}{e \times R_{H_{theo}}} \\
	&= \frac{1}{(1.602 \times 10^{-19}) * (4.8 \times 10^{-3})} \\
         &= 1.30 \times 10^{21}
\end{align*}
...
\Delta &= | n_{exp} - n_{theo} | \\
       &= | 7.21 \times 10^{20} - 1.30 \times 10^{21}| \\
       &= 5.79 \times 10^{20} 
\end{align*}
...
\subsubsection{Conclusion}
%Brief summary, discussion of results and theory
The experimental value of $R_{\text{H}} = \num{0.0087}\si{\cubic\meter\per\ampere\per\second}$ is within 33 $\sigma$ of the theoretical value $R_{\text{H}} 4.8 \times 10^{-3}$ m$^3$/As. After using these values to calculate carrier concentration, the propagated error reached 58 $\sigma$. The tight linear fit indicates this is a consequence of not random errors but systematic errors, including but not limited to the increased  distance of the Hall probe that would not fit in the holder, and the latent field of the coils, having warmed up during previous experiments.
...
\subsubsection{Conclusion}
\qq The plot in Figure (\ref{gph:nt5}) closely resembles the expected distribution. It
...
and the induced magnetic field will remain off. The sample voltage
...
conductivity is related to the inverse temperature by the relation
$\sigma = \sigma_0 e^{-E_g/2kT}$.This will allow one to create a graph
of the conductivity as a function of the inverse
temperature. Furthermore, plotting the natural logarithm of the
...
\qq Several sources of error can be found in this experiment. One such
...
power supply did not keep a steady AC current. The voltmeter had was limited to 1 $\mu$V, which left an uncertainty of the same order of magnitude as the measurements being taken. Thus, some data trends resembled a step function, rather than a continuous curve. Note that random sources of error can be accounted for by propagating error in a linear fit, for example. However, systematic errors were largely unaccounted for in calculations, resulting in some large $\sigma$ discrepancies.
